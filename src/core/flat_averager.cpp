...
Eigen::MatrixXd FlatAverager::pseudoInverse(const Eigen::MatrixXd &A, double tolerance)
{
...
std::vector<double> FlatAverager::getWeights(const std::vector<double> &errors, int model_count) const
{
...
void FlatAverager::validateFlatDimensions(const std::vector<std::unique_ptr<FlatModel>> &models) const
{
    ...
    for (size_t i = 1; i < models.size(); ++i)
    {
        ...
        {
            throw std::runtime_error("FlatAverager: All models must live in the same ambient space.");
        }
    }
}

void FlatAverager::fit(const Eigen::MatrixXd &D)
{
}
void FlatAverager::fit(const std::vector<std::unique_ptr<FlatModel>> &models, const std::vector<double> &errors)
{
    ...
    std::vector<Eigen::MatrixXd> Q_list;
    ...
    computeAggregatedQr(Q_list, r_list, Q_star, r_star, weights);
